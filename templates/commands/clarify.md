...
- 然后立即将澄清应用到最合适的部分：
  ...
  * 术语冲突 → 在规格中统一术语；如必要保留原术语，在首次使用处添加 `（曾称为"X"）`
- 如果澄清使早期的模糊陈述失效，替换该陈述而不是重复；不留过时的矛盾文本
  * **示例**：如果原规格说"视觉风格待定"，澄清后确定为"Q版3D动画"，则删除"待定"表述，直接替换为"Q版3D动画，2:3头身比例，大眼睛圆脸，参考《摩尔庄园》风格"
...
- 覆盖度摘要表，列出每个分类体系类别及状态：
  ...
  **示例表格格式**：
  ...
  | 维度 | 状态 | 说明 |
  |------|------|------|
  | 1. 视觉风格与美术方向 | 已解决 | Q1: 确定为Q版3D动画，2:3头身比例 |
  | 2. 角色设计与一致性 | 已解决 | Q2: 选择13图融合策略，创建角色素材库 |
  | 3. 场景描述与环境 | 明确 | 已有完整的三层画面结构定义 |
  | 4. 叙事结构与情绪弧线 | 已解决 | Q3: 选择三幕结构，定义6场景情绪弧线 |
  | 5. 镜头语言与构图 | 明确 | 已明确镜头类型和相机运动需求 |
  | 6. 光照与色彩 | 延期 | 超出5问配额，在L1验证时调整 |
  | 7. 音频与对话 | 已解决 | Q4: 确定需要唇形同步，选择即梦AI平台 |
  | 8. 平台适配与技术约束 | 已解决 | Q5: 目标平台抖音，9:16竖屏，60秒 |
  | 9. 预算与成本控制 | 明确 | 预算150-200元，质量与成本平衡 |
  | 10. 边缘情况与合规性 | 未解决 | 影响较低，可在实施阶段处理 |
