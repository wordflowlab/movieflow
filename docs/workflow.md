...
1. **规格定义**（/specify）：定义视频的整体规格（主题、风格、目标）
2. **计划制定**（/plan）：将规格细化为技术实现方案
3. **任务分解**（/tasks）：将计划分解为具体的生成任务
4. **视频生成**（/implement）：执行任务，生成视频内容

## 工作流程概览（含渐进式验证）🆕

```mermaid
graph TD
    A[用户描述] --> B[/specify<br/>视频规格]
    B --> C[/plan<br/>技术方案]
    C --> D[/validate<br/>L0+L1验证]
    D --> E{验证通过?}
    E -->|否| B
    E -->|是| F[/tasks<br/>任务分解]
    F --> G[/preview<br/>L2动态预览]
    G --> H{效果满意?}
    H -->|否| B
    H -->|是| I[/implement<br/>完整生成]
    I --> J[质量检查]
    J --> K[发布输出]
...
```bash
/preview health-app-intro --scene 1 --with-audio
```
