...
sequenceDiagram
    ...
    User->>CLI: /implement --project tang-monk
    CLI->>CLI: 解析6个10秒场景
    CLI->>CLI: 分成2批处理(每批3个)
...
#### Step 1: 创建项目规格
```bash
/specify 创建一个Q版唐僧相亲自我介绍视频，幽默风格，60秒，包含6个场景
```

#### Step 2: 制定技术方案
```bash
/plan tang-monk-dating
```
...
#### Step 4: 生成任务列表
```bash
/tasks tang-monk-dating
```

#### Step 5: 执行视频生成
```bash
/implement tang-monk-dating --platform douyin
```
